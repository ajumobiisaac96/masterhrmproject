...
//   faRightFromBracket, faPlus, faEdit, faChartBar 
...
//             <>
//               {/* 🔹 Profile Section */}
//               <div style={{ display: "flex", alignItems: "center", gap: "15px", padding: "20px", background: "#fff", borderRadius: "10px", boxShadow: "0px 2px 10px rgba(0, 0, 0, 0.1)" }}>
//                 <img src={adminData?.profile_image || test} alt="Admin Profile" style={{ width: "80px", height: "80px", borderRadius: "50%" }} />
//                 <div>
//                   <h3 style={{ margin: 0 }}>{adminData?.first_name} {adminData?.last_name}</h3>
...
//                  {[
//                     { icon: faPlus, label: "Add Employees", color: "#007BFF" },
//                     { icon: faEdit, label: "Edit Department", color: "#FFC107" },
//                     { icon: faChartBar, label: "View Report", color: "#28A745" }
//                   ].map((action, index) => (
//                     <div key={index} style={{
...
const handleSaveImage = async () => {
  ...
  formData.append("image_file", file);  // Make sure 'image_file' is the correct name expected by your backend

  // Start the saving process
  setIsSaving(true);  // Set saving state to true to show "Saving..." message
  ...
  try {
    ...
    // Update the adminData state with the new profile image
    setAdminData((prevData) => ({
      ...prevData,
      profile_image: response.data.profile_image,  // Update with new image URL from response
    }));

    // Refresh the page or update UI here to show the new profile image
    // Optionally, you can force a page refresh, or just set the state as done above
    ...
    setIsSaving(false);  // Stop the "Saving..." process in case of an error
  }
};
              ...
              {isModalOpen && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {imageUploadStatus && (
                        <p style={{ color: imageUploadStatus.includes("Failed") ? "red" : "green" }}>
                          {imageUploadStatus}
                        </p>
                      )}
