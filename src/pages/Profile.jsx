  ...
  const Profile = () => {
    ...
    useEffect(() => {
      const fetchEmployeeData = async () => {
        if (!employeeId || !companyId || !accessToken) {
          console.error('Missing required authentication details:', { employeeId, companyId, accessToken });
        ...
        try {
          const response = await axios.get(
            `https://proximahr.onrender.com/employee-management/${employeeId}/employee`,
            {
      ...
      fetchEmployeeData();
    }, [employeeId, companyId, accessToken]);
      ...
      <div>
        <div className="main-dashboard">
          ...
          <div className="dashboard">
            ...
            {employeeData && (
              <div className="employee-info">
                <h2><FontAwesomeIcon icon="fa-envelope" className="icon" /> {employeeData.email}</h2>
                <h2><FontAwesomeIcon icon="fa-solid fa-phone" className="icon" /> {employeeData.phone_number}</h2>
                <h2><FontAwesomeIcon icon="fa-solid fa-location-dot" className="icon" /> {employeeData.work_location}</h2>
              </div>
            )}

            <div className="employee-profile-info">
              <button onClick={() => setActiveSection('personalInfo')} className={activeSection === 'personalInfo' ? 'active' : ''}>Personal Information</button>
              <button onClick={() => setActiveSection('compensation')} className={activeSection === 'compensation' ? 'active' : ''}>Compensation</button>
              <button onClick={() => setActiveSection('employeeDetails')} className={activeSection === 'employeeDetails' ? 'active' : ''}>Employment Details</button>
              <button onClick={() => setActiveSection('performanceMetrics')} className={activeSection === 'performanceMetrics' ? 'active' : ''}>Performance Metrics</button>
              <button onClick={() => setActiveSection('attendanceLeave')} className={activeSection === 'attendanceLeave' ? 'active' : ''}>Attendance & Leave</button>
            </div>
            ...
            <div style={{ display: activeSection === 'attendanceLeave' ? 'block' : 'none' }}>
              <AttendanceLeaveOverview />
            </div>
          </div>
        </div>
